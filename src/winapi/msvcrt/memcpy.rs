...
pub fn memcpy(emu: &mut dyn EmulatorEngine) -> Result<(), EmulatorError> {
    // void *memcpy(
    //   void *dest,       // RCX
    //   const void *src,  // RDX
    //   size_t count      // R8
    ...
    let dest = emu.reg_read(X86Register::RCX)?;
    let src = emu.reg_read(X86Register::RDX)?;
    let count = emu.reg_read(X86Register::R8)?;
    ...
    if count == 0 {
        ...
        emu.reg_write(X86Register::RAX, dest)?;
        return Ok(());
    }
