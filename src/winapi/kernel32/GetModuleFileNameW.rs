...
pub fn GetModuleFileNameW(emu: &mut dyn EmulatorEngine) -> Result<(), EmulatorError> {
    ...
    let filename = if h_module == 0 {
        ...
        if let Some(module) = MODULE_REGISTRY.get_loaded_module_by_module_base(h_module) {
            ...
            log::warn!("[GetModuleFileNameW] Unknown module handle: 0x{:x} ERROR_INVALID_HANDLE", h_module);
