...
pub fn GetLocaleInfoW(emu: &mut dyn EmulatorEngine) -> Result<(), EmulatorError> {
    // Get parameters from registers
    let locale = emu.reg_read(X86Register::RCX)? as u32;
    let lctype = emu.reg_read(X86Register::RDX)? as u32;
    let lp_lc_data = emu.reg_read(X86Register::R8)?;
    let cch_data = emu.reg_read(X86Register::R9)? as u32;
