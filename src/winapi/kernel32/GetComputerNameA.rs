...
pub fn GetComputerNameA(emu: &mut dyn EmulatorEngine) -> Result<(), EmulatorError> {
    // BOOL GetComputerNameA(
    ...
    log::info!("[GetComputerNameA] lpBuffer: 0x{:x}", lp_buffer);
    log::info!("[GetComputerNameA] nSize: 0x{:x}", n_size);
    
    // Check for NULL nSize pointer
    if n_size == 0 {
        log::error!("[GetComputerNameA] NULL nSize pointer");
    ...
    log::info!("[GetComputerNameA] Buffer size: {} characters", buffer_size);
    
    // Mock computer name (NetBIOS name - typically uppercase and limited to 15 chars)
    let computer_name = "TESTCOMPUTER";
    let computer_name_len = computer_name.len() as u32;
    let required_buffer_size = computer_name_len + 1; // Include null terminator
    
    // Check if buffer is large enough
    if buffer_size < required_buffer_size {
        log::warn!("[GetComputerNameA] Buffer too small: need {} characters, got {}", 
    ...
    if lp_buffer != 0 {
        // Write the computer name string including null terminator
        let computer_name_bytes = computer_name.as_bytes();
        emu.mem_write(lp_buffer, computer_name_bytes)?;
        emu.mem_write(lp_buffer + computer_name_bytes.len() as u64, &[0u8])?; // Null terminator
        
        log::info!("[GetComputerNameA] Wrote computer name: '{}'", computer_name);
    }
    
    // Write actual length to nSize (NOT including null terminator, per API spec)
    let actual_size_bytes = computer_name_len.to_le_bytes();
    emu.mem_write(n_size, &actual_size_bytes)?;
    
    log::warn!("[GetComputerNameA] Mock implementation - returned computer name: '{}'", computer_name);
