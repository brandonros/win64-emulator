...
pub fn SetThreadDescription(emu: &mut dyn EmulatorEngine) -> Result<(), EmulatorError> {
    // HRESULT SetThreadDescription(
    //   [in] HANDLE hThread,              // RCX
    //   [in] PCWSTR lpThreadDescription   // RDX
    // );
    
    let h_thread = emu.reg_read(X86Register::RCX)?;
    let description_ptr = emu.reg_read(X86Register::RDX)?;
