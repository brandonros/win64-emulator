...
pub fn code_hook_callback<D>(emu: &mut Unicorn<D>, addr: u64, size: u32) {
    ...
    if is_winapi_call {
        // Look up which IAT function this address belongs to
        let function_info = IAT_FUNCTION_MAP.read().unwrap().get(&addr)
            .map(|(dll, func)| format!("{}!{}", dll.to_ascii_lowercase(), func))
            .unwrap();

        if function_info == "kernel32.dll!GetSystemTimeAsFileTime" {
            // This shouldn't happen anymore since we're resolving to actual DLL address
            log::warn!("GetSystemTimeAsFileTime still hitting mock address at 0x{:016x}", addr);
            // Don't panic, let execution continue
        } else { 
            log::error!("🚨 Reached unmocked WinAPI IAT call: {}", function_info);
            log::error!("   Address: 0x{:016x}", addr);
            log::error!("   This function needs to be implemented or mocked");
            
            log::logger().flush();
            panic!("reached winapi_call without iat hook/mock at 0x{:x} ({})", addr, function_info);
        }
    }
