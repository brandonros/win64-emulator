...
use crate::pe::relocations::process_relocations;
...
impl ModuleRegistry {
    ...
    pub fn load_system_dll(&self, emu: &mut Unicorn<()>, dll_path: &str, dll_name: &str, desired_base: Option<u64>) -> Result<(), String> {
        ...
        // Then load the DLL sections properly (overwriting with the correct section data)
        for section in dll_pe.sections() {
            if !section.raw_data().is_empty() {
                // The section virtual address is relative to image base in the LoadedPE
                // We need to adjust it to the allocated base address
                let section_va = section.virtual_address() - dll_pe.image_base() + base_addr;
                emu.mem_write(section_va, section.raw_data()).unwrap(); // TODO: not unwrap
                log::info!("  Loaded section '{}' at 0x{:016x} (size: 0x{:x})", 
                         section.name(), section_va, section.raw_data().len());
            }
        }
        
        // Process relocations if the DLL is loaded at a different base than expected
        if base_addr != dll_pe.image_base() {
            log::info!("  Processing relocations (delta: 0x{:x})", 
                     base_addr as i64 - dll_pe.image_base() as i64);
            if let Err(e) = process_relocations(emu, &dll_pe, base_addr) {
                log::warn!("  Failed to process relocations: {}", e);
            }
        }
        
        // Build export map with actual DLL export addresses
        ...
        for (name, export) in dll_pe.exports() {
            // Convert export address to RVA if it's an absolute address
            let export_rva = if export.address >= dll_pe.image_base() {
                // It's an absolute address, convert to RVA
                export.address - dll_pe.image_base()
            } else {
                // It's already an RVA
                export.address
            };
            
            // Calculate the actual address in our loaded DLL
            let actual_addr = base_addr + export_rva;
            dll_exports.insert(name.clone(), actual_addr);
            
            // Add to IAT function map for hook interception (if still needed)
            iat_map.insert(
                actual_addr,
                (dll_name.to_string(), name.clone())
            );
            
            log::info!("  {}!{}: export @ 0x{:x} -> RVA 0x{:x} -> actual 0x{:x}", 
                     dll_name, name, export.address, export_rva, actual_addr);
        }
